...
namespace DAL
{
    public  class ASP_RepairerProductInfo:Sys_InitParentInfo
    {
          private int MethodID;

        public ASP_RepairerProductInfo()
        {
            this.MethodID = 0;
        }

        public ASP_RepairerProductInfo(int MenthodID)
        {
        ...
        public WebReturn Search(Model.Sys_UserInfo user, Model.ReportPagingParam pageParam)
        {
            using (LinQSqlHelper lqh = new LinQSqlHelper())
            {
                try
                {
                    ...
                    var aduit_query = from b in lqh.Umsdb.GetTable<Model.ASP_RepairerProductInfo>()
                                      select b;
                    #endregion
                    ...
                    if (pageParam.PageIndex > pagecount)
                    {
                        ...
                        List<Model.ASP_RepairerProductInfo> aduitList = results.ToList();
        ...
        /// <param name="dl"></param>
        /// <returns></returns>
        public Model.WebReturn Add(Model.Sys_UserInfo user, List<Model.ASP_RepairerProductInfo> list)
        {
            using (LinQSqlHelper lqh = new LinQSqlHelper())
            {
                using (TransactionScope tx = new TransactionScope())
                {
                    ...
                    {
                        var query = from b in lqh.Umsdb.GetTable<Model.ASP_RepairerProductInfo>()
                                    where list.Select(a=>a.RepairerID).Contains(b.RepairerID)==false
                                    select b;
                        if (query.Count() > 0)
                        {
                            ////新增
                            //Model.ASP_RepairerProductInfo a = new Model.ASP_RepairerProductInfo();
                            //a.RepairerID=
                        }
        ...
        /// <param name="d"></param>
        /// <returns></returns>
        public Model.WebReturn Delete(Model.Sys_UserInfo user, int rid)
        {
            using (LinQSqlHelper lqh = new LinQSqlHelper())
            {
                using (TransactionScope tx = new TransactionScope())
                {
                    try
                    {
                        ...
                        if (query.Count() == 0)
                        {
                            return new Model.WebReturn() { ReturnValue = false, Message = "未能找到数据！" };
                        }
