...
namespace DAL
{
    ...
    public class BorowAduit3
    {
        ...
        public BorowAduit3(int MenthodID)
        {
                ...
                using (TransactionScope ts = new TransactionScope())
                {
                    try
                    {
                        ...
                        foreach (var item in models)
                        {
                            ...
                            {
                                return new Model.WebReturn() { ReturnValue = false, Message = "审批单 "+aduitModel.AduitID+" 未进行二级审批，审批失败！" };
                            }
                            ...
                            aduitModel.AduitDate3 = item.AduitDate3;
                            aduitModel.AduitUser3= item.AduitUser3;
                            aduitModel.Aduited3 = item.Aduited3;
                            aduitModel.Note3 = item.Note3;
                            aduitModel.Aduited3 = true;
                            aduitModel.Passed3 = item.Passed3;

                            if (item.Passed3 != true || Convert.ToDecimal(aduitModel.TotalMoney) <= Convert.ToDecimal(user.BrwLimit))
                            {
                                aduitModel.Aduited = true;
                                aduitModel.Passed = item.Passed3;
        ...
        public Model.WebReturn Search(Model.Sys_UserInfo user, Model.ReportPagingParam pageParam)
        {
            using (LinQSqlHelper lqh = new LinQSqlHelper())
            {
                try
                {
                    ...
                    var aduit_query = from b in lqh.Umsdb.View_BorowAduit3
                    ...
                    if (pageParam.PageIndex > pagecount)
                    {
                        ...
                        List<Model.View_BorowAduit3> aduitList  = results.ToList();
