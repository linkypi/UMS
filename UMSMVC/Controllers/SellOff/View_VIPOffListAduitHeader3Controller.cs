...
{
    public class View_VIPOffListAduitHeader3Controller : BasePage
    {
        ...
        public IQueryable<ReportModel.View_VIPOffListAduitHeader> Get()
        {
            try
            {
                ...
                imeis = (from b in imeis
                         where ((b.Aduited3) == null || b.Aduited3 != "是") && b.Aduited2=="是"
                         select b);
        ...
        public void GetSellOffUnAduitFirst()
        {
            try
            {
                ...
                bool passed3 = Convert.ToBoolean(System.Web.HttpContext.Current.Request.Form["passed3"]);
                string Note3 = System.Web.HttpContext.Current.Request.Form["Note3"] + "";
                
                DAL.VIP_OffList aduit = new DAL.VIP_OffList(Model.MethodIDListInfo.Pro_SellOff_Aduit3);
                Model.WebReturn r = aduit.Aduit3(user, ID, passed3, Note3);
                ////return imeis.AsQueryable();
        ...
        public IQueryable<ReportModel.View_VIPOffListAduitHeader> GetSellOffAduitFirstNext(int ID)
        {
            try
            {
                ...
                imeis = (from b in imeis
                             orderby b.ID
                         where b.ID > ID && ((b.Aduited3) == null || b.Aduited3 != "是") && b.Aduited2 == "是"
                             select b);
                if (imeis.Count() == 0)
                {
                    imeis = (from b in imeis
