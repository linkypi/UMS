...
{
    public class Report_SellAduitInfo3Controller : BasePage
    {
        //
        // GET: /View_BorowAduit/
        ...
        public IQueryable<Model.Report_SellAduitInfo> Get()
        {
            try
            {
                var imeis = (from b in context.Report_SellAduitInfo
                             where (b.二级已审批) == "Y" && (b.已审批 == null || b.已审批 == "N")
                             && (b.三级已审批 == null || b.三级已审批 == "N")
                             select b);
        ...
        public void GetSellAduitUnAduitFirst(Model.Report_SellAduitInfo BrwAduit)
        {
            try
            {
                ...
                DAL.Pro_SellAduit aduit = new DAL.Pro_SellAduit(Model.MethodIDListInfo.Pro_SellAduit3_Aduit);
                Model.WebReturn r= aduit.Aduit3(user, new List<Model.Pro_SellAduit>() { brw },false);
                //return imeis.AsQueryable();
        ...
        public IQueryable<Model.Report_SellAduitInfo> GetSellAduitAduitFirstNext(int ID)
        {
            try
            {
                ...
                var imeis = (from b in context.Report_SellAduitInfo
                             orderby b.系统主键
                             where b.系统主键 > ID && (b.二级已审批) == "Y" && (b.已审批 == null || b.已审批 == "N")
                             && (b.三级已审批 == null || b.三级已审批 == "N")
                             select b);
                if (imeis.Count() == 0)
                {
