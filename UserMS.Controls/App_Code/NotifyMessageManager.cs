...
namespace UserMS
{
    public class NotifyMessageManager
    {
        ...
        private RadTabControl _Tab;
        private delegate void MethodInvoker();

        public NotifyMessageManager(double screenWidth, double screenHeight, double popupWidth, double popupHeight, RadTabControl Tab)
        {
                    ...
                    if (nextLocation > -1)
                    {
                        ...
                        if (QueuedMessages.TryDequeue(out msg))
                        {
                            ...
                            dispatcher.BeginInvoke(
                                new MethodInvoker(() => 
                                {
                                    var window = new NotifyMessageWindow(this._Tab) 
                                    { 
