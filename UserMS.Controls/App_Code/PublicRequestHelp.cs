...
namespace UserMS
{
    public class PublicRequestHelp
    {
        ...
        public static bool IsLogin()
        {
            ...
            if (Store.LoginUserInfo == null||Store.wsclient == null || Store.wsclient.State != CommunicationState.Opened)
            {
#if HZ
                var a=Assembly.LoadFrom("UserMS_HZ.exe");
#else
                var a = Assembly.LoadFrom("UserMS.exe");
#endif
                Type T = a.GetType("UserMS.Login");

                object Login = Activator.CreateInstance(T, new object[] {  });//创建实例
        ...
        public static void InitStore(List<object> arrList)
        {
            ...
            Store.Factorys = (arrList[30] == null) ?
              (List<API.ASP_Factory>)Store.GetClientStore("Factory") :
          (List<API.ASP_Factory>)arrList[30] ?? new List<API.ASP_Factory>();

            Store.Dealers = (arrList[31] == null) ?
              (List<API.ASP_Dealer>)Store.GetClientStore("Dealers") :
          (List<API.ASP_Dealer>)arrList[31] ?? new List<API.ASP_Dealer>();

            Store.RepairerProductInfo = (arrList[32] == null) ?
              (List<API.ASP_RepairerProductInfo>)Store.GetClientStore("RepairerProductInfo") :
          (List<API.ASP_RepairerProductInfo>)arrList[32] ?? new List<API.ASP_RepairerProductInfo>();
            ...
            Store.SetClientStore("Factory", Store.Factorys);
            Store.SetClientStore("Dealers", Store.Dealers);
            Store.SetClientStore("RepairerProductInfo", Store.RepairerProductInfo);
