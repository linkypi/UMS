...
namespace UserMS.Common
{
    ...
    public class ProDetailAdder<T> where T : class, new()
    {
        ...
        public ProDetailAdder(List<T> Models, RadGridView ProGrid, List<API.BJModel> pros)
        {
            Init(Models, ProGrid, pros);

        }

        public ProDetailAdder(List<T> Models, RadGridView ProGrid, List<API.BJModel> pros, EventHandler<MyEventArgs> addCompleted)
        {
        ...
        private void Init(List<T> Models, RadGridView ProGrid, List<API.BJModel> pros)
        {
            ...
            pros = pros ?? new List<API.BJModel>();
            ...
            foreach (var item in query)
            {
                ...
                bm.IsDecimal = Convert.ToBoolean(item.IsDecimal);
        ...
        public void Add()
        {
            msFrm = new NoFileterMulSelect(
               treeModels,
               models, "ProName",
               new string[] { "ClassName", "TypeName", "ProName", "ProFormat", "NeedIMEI","IMEI","InListID" },
               new string[] { "商品品牌", "商品类别", "商品型号", "商品属性", "需要串码","串码","商品批次" });
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        public void Add(string[] arr1, string[] arr2)
        {
            msFrm = new NoFileterMulSelect(
               treeModels,
               models, "ProName",
        ...
        /// <returns></returns>
        private bool ValidateProc(SlModel.BaseModel item)
        {
            // List<T> list = GridUnCheckPro.selete
            foreach (var vm in oldmodels)
            {
                object obj  = vm.GetType().GetProperty("IMEI").GetValue(vm, null);
                string imei = obj==null?"":obj.ToString();
                if (imei==item.IMEI && item.NeedIMEI)
                {
                    return true;
                }
            }
