...
{
    public class SellChartModel : INotifyPropertyChanged
    {
        ...
        public  void calcnewresult()
        {
            ...
            for (int i = StartDay; i <= EndDay; i++)
            {
                results.Add(new UserMS.DemoReport.SellChartModel()
                    {
                        Day = i,
                        M1 = Convert.ToDecimal(m1.Any(p => p.DATE.Day == i) ? m1.Where(p => p.DATE.Day == i).Sum(p => p.Sells) : 0),
                        M2 =  Convert.ToDecimal(m2.Any(p => p.DATE.Day == i) ? m2.Where(p => p.DATE.Day == i).Sum(p => p.Sells) : 0),
                    });
