...
namespace UserMS.Report.Print
{
    ...
    /// </summary>
    public partial class PrintAssetsOutOrder : BasePage
    {
        private int PageSize = 20;
        private string ReportName = "Report_OutInfo";
        public PrintAssetsOutOrder()
        {
            InitializeComponent();
        ...
        private void BasePage_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                this.datasource.Expand = "Report_OutOrderIMEIInfo";
                ...
                reportDataSource.Name = "AssetsOutDataSet"; // Name of the DataSet we set in .rdlc

                _reportViewer.LocalReport.DisplayName = "资产调拨单";
                _reportViewer.LocalReport.ReportPath = "Report\\Print\\Assetstransfer.rdlc"; // Path of the rdlc file
        ...
        private void Grid_SelectionChanged(object sender, SelectionChangeEventArgs e)
        {
            ...
            List<ReportService.Report_OutOrderIMEIInfo> imeis = new List<Report_OutOrderIMEIInfo>();

            foreach (var item in this.Grid.SelectedItems)
            {
                var child= (ReportService.Report_OutInfo)item;
                for(int i=0; i<=200;i++)
                {
                    imeis.AddRange(child.Report_OutOrderIMEIInfo);
                }
            }

            reportDataSource.Value = imeis;
