...
namespace UserMSMobileAPI
{
    ...
    public partial class JsonAPI : System.Web.UI.Page
    {
        ...
        public bool CheckArgs()
        {
            ...
            return true;
            if (SignClass.CheckArgs(Request.QueryString))
            {

                if (SignClass.CheckSign(Request.QueryString))
                {
                    try
                    {
                        ...
                        if (System.Math.Abs(SignClass.GetUnixTime(DateTime.Now) - long.Parse(Request.QueryString["timestamp"]) / 1000) > 10 * 60)
                        {
                        ...
                        Logger.writelog(Request.Url.OriginalString, Request.ServerVariables["REMOTE_ADDR"]);
                        return true;
        ...
        public void OutputJson()
        {
            ...
            {
                json = string.Format("{0}({1})", Request.QueryString["callback"], json);
            }
            Response.Write(json);
